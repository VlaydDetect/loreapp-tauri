...
impl SurrealStore {
    pub(in crate::model) async fn new() -> Result<Self> {
        let ds = Datastore::new(format!("file://{}/DB.db", path_to_string(&get_user_path())).as_str()).await?;
    ...
    pub(in crate::model) async fn exec_create<T: Creatable>(&self, tb: &str, data: T) -> Result<String> {
    ...
    pub(in crate::model) async fn exec_merge<T: Patchable>(&self, tid: &str, data: T) -> Result<String> {
    ...
    pub(in crate::model) async fn exec_select<O: Into<FilterGroups>>(&self, tb: &str, filter_groups: Option<O>, list_options: ListOptions) -> Result<Vec<Object>> {
