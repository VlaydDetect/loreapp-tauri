use super::{
    ...
    common::{closeparentheses, commas, openparentheses, verbar},
    fmt::Fmt,
    table::{table, Table},
};
use nom::{
...
fn option(i: &str) -> IResult<&str, Kind> {
    ...
    let (i, _) = char('<')(i)?;
    let (i, v) = map(
        alt((either, simple, geometry, record, array, set)),
        Box::new,
    )(i)?;
    let (i, _) = char('>')(i)?;
    Ok((i, Kind::Option(v)))
...
mod tests {
    ...
    fn kind_record_many() {
        ...
        assert_eq!(
            out,
            Kind::Record(vec![Table::from("person"), Table::from("animal")])
        );
    }
    ...
    fn kind_geometry_many() {
        ...
        assert_eq!(
            out,
            Kind::Geometry(vec![String::from("point"), String::from("multipoint")])
        );
    }
    ...
    fn kind_option_many() {
        ...
        assert_eq!(
            out,
            Kind::Option(Box::new(Kind::Either(vec![Kind::Int, Kind::Float])))
        );
    }
