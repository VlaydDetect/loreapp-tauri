...
impl Display for MissingVariable {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        write!(f, "Unable to resolve variable ${}", self.variable)
    }
}
...
pub(crate) fn resolve_path(raw: &str, get_ent: impl Fn(&str) -> Option<String>) -> Result<PathBuf, Box<dyn Error>> {
    ...
    while let Some(dollar_sign) = unprocessed.find('$') {
        ...
        match parse_identifier(&tail[1..]) {
            Some((variable, rest)) => {
                let value = get_ent(variable).ok_or_else(|| MissingVariable {
                    variable: variable.to_string()
                })?;
                resolved.push_str(&value);
                unprocessed = rest;
            }
            None => return Err(UnableToParseVariable { rest: tail.into() }.into())
        }
    }
