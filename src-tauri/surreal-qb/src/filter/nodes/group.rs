...
mod surrealql {
	...
	use crate::error::{SurrealResult, IntoSurrealError};

	impl TryFrom<FilterGroup> for Condition {
		type Error = IntoSurrealError;
		fn try_from(val: FilterGroup) -> SurrealResult<Self> {
			// Note: this will fail on first, error found
			let exprs: SurrealResult<Vec<Vec<ConditionExpression>>> =
				val.into_iter().map(|node| node.into_surql_expr_list()).collect();
	...
	impl TryFrom<FilterGroups> for Condition {
		type Error = IntoSurrealError;
		fn try_from(val: FilterGroups) -> SurrealResult<Condition> {
	...
	impl FilterGroups {
		pub fn into_surreal_condition(self) -> SurrealResult<Condition> {
			let mut cond = Condition::any();
