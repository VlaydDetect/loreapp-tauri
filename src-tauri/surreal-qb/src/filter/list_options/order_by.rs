...
mod surralql {
	...
	impl OrderBys {
		pub fn into_surrealql_order_iter(self) -> impl Iterator<Item = (String, crate::Order)> {
			self.0.into_iter().map(OrderBy::into_surrealql_order)
		}
	}

	impl OrderBy {
		pub fn into_surrealql_order(self) -> (String, crate::Order) {
			let (prop_name, order) = match self {
				OrderBy::Asc(prop) => (prop.clone(), crate::Order::Asc),
				OrderBy::Desc(prop) => (prop.clone(), crate::Order::Desc),
			};
