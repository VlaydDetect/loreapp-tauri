...
//! ## Attributes on Enums
//!
//! Magic-Utils supports several custom attributes to modify the generated code. At the enum level, the following attributes are supported:
//!
//! - `#[magic(serialize_all = "case_style")]` attribute can be used to change the case used when serializing to and deserializing
...
//!
//!   ```rust
//!   use magic_utils_macros;
//!
//!   #[derive(Debug, Eq, PartialEq, magic_utils_macros::Display)]
//!   #[magic(serialize_all = "snake_case")]
//!   enum Brightness {
//!       DarkBlack,
//!       Dim {
//!           glow: usize,
//!       },
//!       #[magic(serialize = "bright")]
//!       BrightWhite,
//!   }
//!
//!   assert_eq!(
...
//!   ```
//!
//! - You can also apply the `#[magic(ascii_case_insensitive)]` attribute to the enum,
//!   and this has the same effect of applying it to every variant.
//!
//! ## Attributes on Variants
//!
//! Custom attributes are applied to a variant by adding `#[magic(parameter="value")]` to the variant.
//!
...
//!     ```text
//!     // Replaces this:
//!     _ => Err(magic::ParseError::VariantNotFound)
