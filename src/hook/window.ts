...
import { getCurrent, currentMonitor, appWindow } from '@tauri-apps/api/window';
...
export async function useResizeEffect(minWidth = 1750) {
    useEffect(() => {
        async function resizeWindow() {
            ...
            const logicalSize = physicalSize.toLogical(
                scaleFactor ? scaleFactor : await appWindow.scaleFactor(),
            );
