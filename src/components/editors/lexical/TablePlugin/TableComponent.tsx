...
function getCellID(domElement: HTMLElement): null | string {
    ...
    while (node !== null) {
        ...
        if (possibleID != null) return possibleID;
...
function $updateCells(rows: Rows, ids: Array<string>, cellCoordMap: Map<string, [number, number]>, cellEditor: null | LexicalEditor, updateTableNode: (fn2: (tableNode: TableNode) => void) => void, fn: () => void,): void {
    for (const id of ids) {
...
function getSelectedRect(startID: string, endID: string, cellCoordMap: Map<string, [number, number]>,): null | {startX: number; endX: number; startY: number; endY: number} {
    const startCoords = cellCoordMap.get(startID);
    const endCoords = cellCoordMap.get(endID);
    if (startCoords === undefined || endCoords === undefined) return null;
...
function TableCellEditor({cellEditor}: {cellEditor: LexicalEditor}) {
    ...
    if (cellEditorPlugins === null || cellEditorConfig === null) return null;
    ...
    return (
        <LexicalNestedComposer
            ...
            {cellEditorPlugins ? Array.isArray(cellEditorPlugins) ? cellEditorPlugins.map(cell => cell) : cellEditorPlugins : null}
        </LexicalNestedComposer>
    );
}
...
function TableActionMenu({cell, rows, cellCoordMap, menuElem, updateCellsByID, onClose, updateTableNode, setSortingOptions, sortingOptions,}: IActionMenuProps) {
...
function TableCell({cell, cellCoordMap, cellEditor, isEditing, isSelected, isPrimarySelected, theme, updateCellsByID, updateTableNode, rows, setSortingOptions, sortingOptions,}: ICellProps) {
        ...
        return mergeRegister(
            ...
            editor.registerCommand<KeyboardEvent>(
                ...
                (event, targetEditor) => {
                    ...
                    if (!$isRangeSelection(selection) || targetEditor !== cellEditor) return false;
